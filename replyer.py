...
from linepy import FinbotService

from linepy import FinbotLoginService
...
loggedIn = codecs.open('logged.json', 'r', 'utf-8')

run_bot = json.load(loggedIn)
...
def fositive():
	...
	txt1 = ['Target Added', 'Add success', 'Added', 'Has been Promoted']	send1 = random.choice(txt1)
...
def sendImageURL(self, to_, url):

      path = '%s/finbotV5-%i.data' % (tempfile.gettempdir(), randint(0, 9))
